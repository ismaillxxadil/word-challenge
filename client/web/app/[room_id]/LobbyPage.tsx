  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900/50 overflow-hidden">
      ...
      >
        ...
        <span className="flex-1 text-right text-sm font-semibold text-slate-200">
          {title}
        </span>
        {badge && (
          <span
        ...
        <div className="overflow-y-auto flex-1 px-4 py-4 space-y-2">
          ...
          >
            <SettingSlider
              ...
              disabled={!isHost}
              onChange={(v) => onChange({ timePerTurn: v })}
            />
          </SettingSection>
          ...
          <SettingSection
            icon={<Layers size={15} />}
            title="عدد البطاقات"
            badge={`${settings.startingCards}`}
            badgeActive
          >
            <SettingSlider
              label="بطاقات البداية"
              ...
              disabled={!isHost}
              onChange={(v) => onChange({ startingCards: v })}
            />
          </SettingSection>
          ...
          >
            <div className="flex items-center justify-between">
              <span className="text-xs text-slate-400 flex-1">
                السماح باستخدام بطاقة القفل
              </span>
              <Toggle
                enabled={settings.allowLockCard}
                onToggle={() =>
                  onChange({ allowLockCard: !settings.allowLockCard })
                }
                color="bg-cyan-500"
                disabled={!isHost}
              />
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs text-slate-400 flex-1">
                السماح بالطعن في الكلمات
              </span>
              <Toggle
                enabled={settings.allowVar}
            ...
            {settings.allowVar && (
              <div className="space-y-3 pt-2 border-t border-slate-800/50">
                <SettingSlider
                  ...
                  disabled={!isHost}
                  onChange={(v) => onChange({ varExplanationDuration: v })}
                />
                <SettingSlider
                  label="مدة التصويت"
                  ...
                  disabled={!isHost}
                  onChange={(v) => onChange({ varDuration: v })}
                />
              </div>
            )}
          </SettingSection>
          ...
          <SettingSection
            ...
            title="حظر تكرار الكلمات"
            badge={settings.noRepeatWords ? "مفعّل" : "معطّل"}
            badgeActive={settings.noRepeatWords}
          >
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs text-slate-400 flex-1">
                لا يمكن إعادة لعب كلمة سُبق لعبها
              </span>
              <Toggle
                enabled={!!settings.noRepeatWords}
                onToggle={() =>
                  onChange({ noRepeatWords: !settings.noRepeatWords })
                }
                color="bg-orange-500"
                disabled={!isHost}
              />
            ...
            {settings.noRepeatWords && (
              <div className="pt-2 border-t border-slate-800/50">
                <SettingSlider
                  ...
                  disabled={!isHost}
                  onChange={(v) => onChange({ maxRepeatCount: v })}
                />
              </div>
            )}
          </SettingSection>
      ...
      <div className="relative w-full max-w-3xl flex flex-col bg-slate-900/80 backdrop-blur-xl border border-slate-700/50 rounded-3xl shadow-2xl overflow-hidden">
            ...
            <div className="flex items-center gap-2 w-full max-w-xs">
              ...
              >
                {roomLinkCopied ? (
                  <>
                    <Check size={15} />
                    <span className="text-[10px] sm:text-sm"> تم النسخ ! </span>
                  </>
                ) : (
                  <>
                    <Share2 size={15} />
        ...
        <div className="px-4 sm:px-6 py-4 sm:py-5">
          <div className="flex items-center justify-between mb-3 sm:mb-4">
            ...
            <div className="flex items-center gap-2">
              <span
                className={`flex items-center gap-1.5 px-1.5 py-0.5 sm:px-2.5 sm:py-1 text-[12px] sm:text-xs font-bold rounded-full border ${
                  canStart
              ...
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full ${canStart ? "bg-green-400" : "bg-amber-400"}`}
                />
            ...
            {players.map((player) => (
              ...
              >
                ...
                <div className="relative flex-shrink-0 w-10 h-10 sm:w-11 sm:h-11 rounded-xl overflow-hidden border border-slate-700/60 bg-slate-900">
                  ...
                  />
                  <span
                    className={`absolute bottom-0.5 right-0.5 w-2.5 h-2.5 rounded-full border-2 border-slate-900 ${player.socketId ? "bg-green-400" : "bg-red-400"}`}
                  />
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    {player.id === currentPlayerId && (
                      <span className="text-[10px] font-bold text-purple-400 bg-purple-400/10 px-1.5 rounded">
                        أنت
                      </span>
                    )}
                  </div>
